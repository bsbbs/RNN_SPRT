...
# ----------------------------------------------------------------------------
plt.rcParams["font.family"] = "DejaVu Sans"   # Arial is unavailable in this env
LABEL_FS = 12                                  # x / y axis label fontsize
TICK_FS = 10                                   # x / y tick-label fontsize

C = 0.04            # sample cost (project-wide)
SIGMA_REPR = 0.25   # representation-noise std
TAU_EXEC = 0.025    # execution-noise (softmax) temperature
...
# ----------------------------------------------------------------------------
CONDITIONS = [
    ("Canonical",            dict(noise="representation", sigma_repr=0.0,        tau_exec=TAU_EXEC)),
    ("Execution noise",      dict(noise="execution",      sigma_repr=0.0,        tau_exec=TAU_EXEC)),
    ("Representation noise", dict(noise="representation", sigma_repr=SIGMA_REPR, tau_exec=TAU_EXEC)),
    ("Mixed noise",          dict(noise="mixed",          sigma_repr=SIGMA_REPR, tau_exec=TAU_EXEC)),
]

# Row 1 = time unconstrained (infinite horizon); Row 2 = time constrained (finite).
ROWS = [
    ("infinite", "Time unconstrained"),
    ("finite",   "Time constraint (t = 10)"),
]
...
def make_figure():
    ...
    # Single shared x-axis label (avoids per-panel label collisions).
    fig.text(
        0.498, 0.028, "Cumulative evidence (Log LR)",
        ha="center", va="center", fontsize=LABEL_FS,
    )

    # Row descriptors on the far left.
    for r, y_center in zip(range(len(ROWS)), (0.69, 0.30)):
        fig.text(
            0.028, y_center, ROWS[r][1],
            rotation=90, va="center", ha="center", fontsize=11,
        )

    # Shared time-step colorbar.
    cax = fig.add_axes([0.905, 0.11, 0.017, 0.77])
    sm = plt.cm.ScalarMappable(cmap="viridis", norm=plt.Normalize(vmin=1, vmax=MAX_T))
    sm.set_array([])
...
if __name__ == "__main__":
    fig = make_figure()
    fig.savefig("figure4_time_constraint.png", dpi=300)
    fig.savefig("figure4_time_constraint.svg")
