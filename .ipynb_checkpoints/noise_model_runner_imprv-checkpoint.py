...
def build_stimuli(config: ModelConfig) -> StimulusSet:
    ...
    This reproduces the previous interpolation-based construction to machine
    precision while admitting any target log10 LR analytically.
    """
    mu = config.mu
    sigma = config.sigma
    ln10 = np.log(10.0)
...
def default_n_gh(noise: NoiseKind) -> int:
    return 201 if noise == "mixed" else 21
...
def compute_model(
    ...
    n_gh = cfg.n_gh if cfg.n_gh is not None else default_n_gh(noise_kind)
